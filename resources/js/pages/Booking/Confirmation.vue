    ...
    >
        <div class="w-full max-w-md -translate-y-10">
            ...
            >
                ...
                <h1 class="mb-2 text-xl font-medium text-white">
                    Afspraak bevestigd!
                </h1>
                <p class="mb-7 text-sm text-stone-400">
                    Je ontvangt een bevestiging op
                    <strong class="break-all text-white">{{
                        appointment.customer_email
                    }}</strong>
                </p>

                <div
                    class="mb-7 space-y-3 rounded-xl border border-stone-700 bg-stone-950 p-5 text-left"
                >
                    ...
                    >
                        ...
                        <span class="text-right font-medium text-emerald-400"
                            >Betaald ✓</span
                        >
                    </div>
                    <div
                        v-else-if="appointment.payment_status === 'open'"
                        class="flex items-start justify-between gap-3 text-xs"
                    >
                        <span class="text-stone-400">Betaling</span>
                        <span class="text-right font-medium text-amber-400"
                            >In afwachting</span
                        >
                    </div>
                </div>
                ...
                <div
                    v-if="
                        ...
                        appointment.status !== 'cancelled'
                    "
                    class="mt-3 grid gap-2 sm:grid-cols-2 sm:gap-3"
                >
                    <Link
                ...
                <div
                    ...
                    class="mt-3 rounded-md border border-red-500/20 bg-red-500/10 py-3 text-center text-[11px] text-red-300 sm:text-xs"
                >
                ...
                <Link
                    href="/"
                    class="mt-3 block w-full rounded-md border border-stone-700 bg-white py-3 text-center text-[11px] font-medium tracking-[0.18em] text-stone-950 transition-colors hover:bg-stone-200 sm:text-xs"
                >
                    TERUG NAAR HOME
                </Link>
                <Link
                    href="/boeken"
                    class="mt-3 block w-full rounded-md border border-stone-700 py-3 text-center text-[11px] text-stone-300 transition-colors hover:border-stone-500 hover:bg-white/5 sm:text-xs"
                >
