<template>
    ...
    <div class="min-h-screen bg-stone-950 px-4 py-8 text-stone-100 sm:py-12">
        <div
            class="mx-auto max-w-4xl rounded-[1.75rem] border border-stone-700 bg-stone-900/95 p-4 shadow-[0_30px_100px_rgba(0,0,0,0.35)] sm:p-6 md:p-8"
        >
            <!-- Header -->
            <div class="mb-8 text-center sm:mb-10">
                <div
                    class="mb-2 text-xs tracking-widest text-amber-500 uppercase"
                >
                    Online reserveren
                </div>
                <h1
                    class="text-2xl font-medium text-white sm:text-3xl md:text-4xl"
                >
                    Plan jouw bezoek
                </h1>
            ...
            <!-- Stap indicator -->
            <div
                class="mb-8 flex items-center justify-center gap-1.5 sm:mb-10 sm:gap-2"
            >
                <template v-for="n in 4" :key="n">
                    <div
                        class="flex h-7 w-7 items-center justify-center rounded-full text-xs font-medium transition-all"
                        :class="
                            n < step
                    ...
                    <div
                        v-if="n < 4"
                        class="h-px w-6 transition-colors sm:w-8"
            ...
            <div
                class="rounded-[1.5rem] border border-stone-700 bg-stone-950 p-4 shadow-[0_24px_90px_rgba(0,0,0,0.2)] sm:p-6 md:p-7"
            >
                <!-- STAP 1: Dienst -->
                <template v-if="step === 1">
                    <h2
                        class="mb-5 text-sm font-medium tracking-wide text-stone-400 uppercase"
                    >
                        Kies een dienst
                    </h2>
                        ...
                        <button
                            ...
                            :class="
                                form.service_id === s.id
                                    ? 'border-amber-500 bg-white/5'
                                    : 'border-stone-700 hover:border-stone-500 hover:bg-white/5'
                            "
                        >
                            ...
                            <span
                                class="ml-auto shrink-0 text-sm font-medium text-white"
                                >{{ s.price_formatted }}</span
                            >
                        </button>
                    </div>
                </template>
                ...
                <template v-if="step === 2">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        >
                            <div
                                class="h-12 w-12 flex-shrink-0 overflow-hidden rounded-full bg-stone-800"
                            >
                                <img
                                    src="/landing-media/barber-portrait.png"
                                    alt="Barber"
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="mt-1.5 flex flex-wrap gap-1">
                                    <span
                                        v-for="spec in barberSpecialties(b)"
                                        :key="spec"
                                        class="rounded-full bg-stone-700 px-2 py-0.5 text-xs text-stone-300"
                                        >{{ spec }}</span
                                    >
                                </div>
                            </div>
                ...
                <template v-if="step === 3">
                    ...
                    <div class="mb-5">
                        <label
                            for="date"
                            class="mb-2 block text-sm font-medium text-stone-400"
                            >Datum</label
                        >
                        <MonthPicker
                            v-model="form.date"
                    ...
                    <div v-if="form.date">
                        <label
                            class="mb-3 block text-xs tracking-wide text-stone-400 uppercase"
                            >Beschikbare tijden</label
                        >
                        <div
                            v-if="loadingSlots"
                        ...
                        <div
                            ...
                            class="rounded-xl border border-stone-700 bg-stone-900 py-6 text-center text-xs text-stone-400"
                        >
                            Geen beschikbare tijden op deze dag. Kies een andere
                            datum.
                        </div>
                        <div
                            v-else
                        ...
                        >
                            <button
                                ...
                                class="cursor-pointer touch-manipulation rounded-lg border py-2.5 text-xs font-medium transition-all active:scale-[0.99]"
                                :class="
                                    form.time === slot
                ...
                <template v-if="step === 4">
                    ...
                    <!-- Samenvatting -->
                    <div
                        class="mb-6 space-y-2.5 rounded-xl border border-stone-700 bg-stone-900 p-4"
                    >
                        <div
                        ...
                        <div
                            class="flex items-start justify-between gap-3 border-t border-stone-700 pt-2 text-xs"
                        >
                    ...
                    <form @submit.prevent="submitBooking" class="space-y-4">
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            <div>
                                <label
                                    class="mb-1 block text-xs tracking-wide text-stone-400 uppercase"
                                    >Naam *</label
                                >
                                <input
                                    v-model="form.name"
                        ...
                        <div>
                            ...
                            <textarea
                                v-model="form.notes"
                                rows="3"
                                placeholder="Bijv. wensen of allergieën"
                        ...
                        <p
                            ...
                            class="rounded-lg border border-red-100 bg-red-50 p-3 text-xs text-red-500"
                        >
                            {{ errors.starts_at }}
                        </p>
                        <button
...
const selectedService = computed(() =>
    props.services.find(
        (service: Service) => service.id === form.value.service_id,
    ),
);
const selectedBarber = computed(() =>
    props.barbers.find((barber: Barber) => barber.id === form.value.barber_id),
);
...
const availableBarbers = computed(() =>
    form.value.service_id
        ? props.barbers.filter((barber: Barber) =>
              (barber.service_ids ?? []).includes(form.value.service_id),
          )
        : props.barbers,
);
