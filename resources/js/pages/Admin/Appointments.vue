      ...
      <table class="w-full text-xs">
        <thead>
          <tr class="border-b border-stone-700 bg-stone-950/70">
            ...
            <th class="px-4 py-3 text-left font-medium text-stone-500">Dienst</th>
            <th class="px-4 py-3 text-left font-medium text-stone-500">Barber</th>
            <th class="px-4 py-3 text-left font-medium text-stone-500">Datum & tijd</th>
            <th class="px-4 py-3 text-left font-medium text-stone-500">Prijs</th>
            <th class="px-4 py-3 text-left font-medium text-stone-500">Status</th>
            <th class="px-4 py-3"></th>
          </tr>
        </thead>
        <tbody class="divide-y divide-stone-700">
          <tr v-for="a in appointments.data" :key="a.id" class="transition-colors hover:bg-white/5">
            <td class="px-5 py-3.5">
              <div class="font-medium text-white">{{ a.customer_name }}</div>
              <div class="text-stone-500">{{ a.customer_phone }}</div>
            </td>
            <td class="px-4 py-3.5 text-stone-300">{{ a.service_name }}</td>
            <td class="px-4 py-3.5 text-stone-300">{{ a.barber_name }}</td>
            <td class="px-4 py-3.5 text-stone-300">{{ a.starts_at }}</td>
            <td class="px-4 py-3.5 text-stone-300">{{ a.price_formatted }}</td>
            ...
            <td class="px-4 py-3.5">
              <div class="flex gap-1">
                ...
                <button v-if="a.can_cancel" @click="updateStatus(a.id, 'cancelled')"
                  class="rounded border border-red-500/20 px-2.5 py-1 text-xs text-red-300 transition-colors hover:bg-red-500/10">
                  × Annuleer
                </button>
                <button v-if="a.status === 'confirmed'" @click="updateStatus(a.id, 'no_show')"
...
function updateStatus(id: string, status: string) {
  router.patch(`/admin/afspraken/${id}/status`, { status }, { preserveScroll: true })
}
