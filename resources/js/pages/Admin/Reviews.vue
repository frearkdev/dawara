...
function importGoogle() {
    ...
    router.post(
        '/admin/reviews/import-google',
        { place_id: importPlaceId.value || null },
        {
            preserveScroll: true,
            onFinish: () => {
...
<template>
    <AdminLayout title="Reviews">
        ...
        <!-- Google Import -->
        <div
            class="mb-5 overflow-hidden rounded-[1.25rem] border border-stone-700 bg-stone-900 p-5"
        >
            <h3 class="mb-3 text-sm font-medium text-white">
                Google Reviews Importeren
            </h3>
            <p class="mb-4 text-[11px] text-stone-400">
                Klik 'Zoek reviews' om automatisch reviews op te halen via
                Google (gratis, geen API key nodig). Of plak reviews handmatig
                via het tekstvak hieronder.
            </p>

            <form
                @submit.prevent="importGoogle"
                class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end"
            >
                <div class="flex-1">
                    <label
                        class="mb-1 block text-[11px] tracking-wider text-stone-500 uppercase"
                        >Google Place ID (optioneel — voor API)</label
                    >
                    <input
                        v-model="importPlaceId"
                        type="text"
                        placeholder="ChIJ... (alleen nodig als je de betaalde API wilt gebruiken)"
                        class="w-full rounded-lg border border-stone-700 bg-stone-950 px-3 py-2 text-sm text-white placeholder-stone-500 focus:border-stone-500 focus:outline-none"
                    />
                </div>
                <button
                    type="submit"
            ...
            <div class="border-t border-stone-800 pt-4">
                <label
                    class="mb-1 block text-[11px] tracking-wider text-stone-500 uppercase"
                    >Reviews plakken van Google Maps</label
                >
                <form
                ...
                >
                    <textarea
                        ...
                        class="w-full resize-none rounded-lg border border-stone-700 bg-stone-950 px-3 py-2 text-sm text-white placeholder-stone-600 focus:border-stone-500 focus:outline-none"
                    />
            ...
            <div class="border-t border-stone-800 pt-4">
                <label
                    class="mb-1 block text-[11px] tracking-wider text-stone-500 uppercase"
                    >Handmatig review toevoegen</label
                >
                <form
                ...
                >
                    <div class="col-span-2">
                        <input
                            v-model="manual.author"
                            type="text"
                            placeholder="Naam klant"
                            required
                            class="w-full rounded-lg border border-stone-700 bg-stone-950 px-3 py-2 text-sm text-white placeholder-stone-600 focus:border-stone-500 focus:outline-none"
                        />
                    </div>
                    <div>
                        <select
                            v-model.number="manual.rating"
                            required
                            class="w-full rounded-lg border border-stone-700 bg-stone-950 px-3 py-2 text-sm text-white focus:border-stone-500 focus:outline-none"
                        >
            ...
            <div class="overflow-x-auto">
                <div class="min-w-[860px]">
                    ...
                    <div
                        ...
                        :key="r.id"
                        class="grid grid-cols-12 items-center border-b border-stone-800 px-5 py-3.5 transition-colors hover:bg-white/5"
                    >
                        <div
                            class="col-span-2 truncate text-xs font-medium text-white"
                        >
                            {{ r.customer_name }}
                        </div>
                        ...
                        <div
                            class="col-span-1 text-xs font-semibold text-amber-500"
                        >
                            {{ r.rating }}/5
                        </div>
                        <div
                            class="col-span-4 truncate pr-3 text-xs text-stone-300"
                            :title="r.comment"
                        >
                            {{ r.comment || '—' }}
                        </div>
                        <div class="col-span-2">
                            <span
                                v-if="r.visible"
                                class="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium text-emerald-300 ring-1 ring-emerald-500/20"
                                >Zichtbaar</span
                            >
                            <span
                                v-else
                                class="rounded-full bg-stone-700 px-2 py-0.5 text-[10px] font-medium text-stone-400 ring-1 ring-stone-600"
                                >Verborgen</span
                            >
                        </div>
